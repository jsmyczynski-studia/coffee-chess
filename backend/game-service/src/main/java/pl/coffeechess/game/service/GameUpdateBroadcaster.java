...
public class GameUpdateBroadcaster {
    ...
    public void broadcastChat(UUID gameId, ChatMessageDto message) {
        if (gameId == null) {
            return;
        }
        messagingTemplate.convertAndSend(GAME_TOPIC_PREFIX + gameId + CHAT_TOPIC_SUFFIX, message);
    }
}
